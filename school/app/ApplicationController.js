...
//-----------Faculty Details--------------
	$scope.GetFacultyDetails = function(){
        ...
        Data.get('action=get_FacultyDetails').then(function (results) {
            Data.toast(results);
...
//----------Faculty ------------------
    ...
    $scope.addEditFaculty = function (p,size) {
        var modalInstance = $modal.open({
          templateUrl: 'partials/addEditFaculty.html',
          controller: 'addEditFacultyCtrl',
          size: size,
          resolve: {
            item: function () {
              return p;
        ...
        modalInstance.result.then(function(selectedObject) {
			if(selectedObject.save == "update"){
                p.faculty_first_name  = selectedObject.faculty_first_name;
                p.faculty_last_name   = selectedObject.faculty_last_name;
				p.faculty_address		= selectedObject.faculty_address;
            }
			if(selectedObject.save == "insert"){
               $scope.commonArrayList.push(selectedObject);
               $scope.commonArrayList = $filter('orderBy')($scope.commonArrayList, 'faculty_first_name','0');
            }
			
        });
    };

    $scope.deleteFaculty = function(objToDelete){
        if(confirm("Are you sure to remove the Faculty ( " + objToDelete.faculty_id + " )"))
        {
			var delSqlStatement = "action=delete_Faculty&faculty_id=" +objToDelete.faculty_id; 
            Data.delete(delSqlStatement).then(function(results){
				Data.toast(results);
                if (results.status == "success") {                    
                    $scope.commonArrayList = _.without($scope.commonArrayList, objToDelete);
                    $scope.commonArrayList = $filter('orderBy')($scope.commonArrayList, 'faculty_first_name','0');                    
                }
            });
        }
...
app.controller('addEditFacultyCtrl', function ($scope, $modalInstance, item, Data) {
	$scope.objAddEditData = new Object();
    ...
    $scope.buttonText =  'Add / Update Faculty';
    var original = item;
    $scope.isClean = function() {
        return angular.equals(original, $scope.objAddEditData);
    }
	
    $scope.saveFaculty = function (objDataToSave) {
		if(objDataToSave.facultyId  != null){
			Data.post('action=updt_Faculty', objDataToSave).then(function (result) {
				Data.toast(result);
				if(result.status == 'success'){
					var x = angular.copy(objDataToSave);
					x.save = 'update';
			...
			Data.post('action=add_Faculty', objDataToSave).then(function (result) {
				Data.toast(result);
				if(result.status == 'success'){
					var x = angular.copy(objDataToSave);
					x.faculty_id = result.faculty_id;
					x.save = 'insert';
