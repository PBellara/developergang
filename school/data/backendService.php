<?php 
    ...
    switch($_GET['action']) 
    {
		...
		case 'get_FacultyDetails':{
			$sql = "select * from faculty order by faculty_first_name";
			$response = executeSQLStatement($sql);
			echo json_encode($response);
        	break;
		}
		...
		case 'delete_Faculty':{
			$whereCondition= "faculty_id =". $_GET['faculty_id'];
            $response = executeDeleteStatement("faculty",$whereCondition);
			echo json_encode($response);
			break;
		}			
...
/*
	...
	function updateFaculty(){
        ...
        $sql .= "faculty_first_name     ='". $updateDetails->facultyFirstName  . "',";
        $sql .= " faculty_last_name      	= ". $updateDetails->facultyLastName   . ",";
        $sql .= " faculty_address      	= ". $updateDetails->facultyAddress   . ",";
        $sql .= " where faculty_id   	=" . $updateDetails->faculty_id ;
        $userRec  = $db->exec($sql); 
        if ( $userRec > 0) {
            $response['status']     = "success";
            $response['title']      = "Success";
            $response['message']    = 'Faculty have been updated successfully';
        }else{
            $response['status']  = "error";
            $response['title']   = "Error";
            $response['message'] = 'Update Faculty failed';
        }
        echo json_encode($response);
	...
	function addFaculty(){
		...
		$sql  = "INSERT INTO faculty(faculty_first_name,faculty_last_name,faculty_address";
		$sql .= ")VALUES(";
        $sql .= "'" . $insertDetails->facultyFirstName . "',";
        $sql .= "'" . $insertDetails->facultyLastName . "',";
        $sql .= "'" . $insertDetails->facultyAddress . "')";
        $insertRec  = $db->query($sql);
        if ( $insertRec != NULL ) {
            $response['faculty_id'] = $db->lastInsertId('faculty_id');
            $response['status']     = "success";
            $response['title']      = "Success";
            ...
            $response['status']  	= "error";
            $response['title']    	= "Error";
            $response['message']  	= "Unable to add Faculty";
        }
		$response["SQL"] = $sql;
        echo json_encode($response);
	}
	...
	function login(){
        ...
        if($userPass == 'admin' && $userEmail == 'admin')   {
            $response['status']     = "success";
            $response['title']      = "Success";
